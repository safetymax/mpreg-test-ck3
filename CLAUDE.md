...
16/18/20. Each one was confidently declared fixed, and the first two were
wrong. That's recorded honestly rather than cleaned up in hindsight,
because the progression of wrong theories is itself the useful part for
next time.
...
(liege to vassal/courtier or close family) to become or be barred from
...
common/character_interactions/mpreg_carrier_interactions.txt
    Force/Forbid Carrier: order another character (liege to vassal/courtier
...
events/mpreg_events.txt
    ...
    0009/0007: the full male-male conception-to-pregnancy-to-birth chain,
...
**Lesson 11: forcing clothing means don't reopen a vanilla portrait_modifiers
...
check with weight ~200-1000, no outfit_tags, no reopening. That's proof
this shape of entry reliably forces clothing in the live game.
...
- Same-sex marriage is already a real vanilla game rule,
  ...
  That's the wrong tool here, since it destroys the shunned/illegal
  distinction the design wants to keep, so it was left untouched.
...
`pregnancy.0001 → pregnancy.1001/2050` pattern, which sidesteps any question of
...
error. It looked exactly like the file was empty of "character"/"father"
...
- `set_variable`'s value field accepts "any event target" (confirmed
  ...
  confirmed one way or the other). This was sidestepped entirely by using
...
- **`any_spouse` used as an *effect*, not a trigger. Real error:
  ...
  (`"Invalid event id"` at their `on_action` hookups). That meant the
  ...
  both `any_spouse` to `random_spouse` occurrences. **Takeaway: brace
...
- **`mode = replace` is invalid for special/morph genes. Real error:
  ...
  `gene_bs_pregnant`, a special gene, and silently never actually suppressed
...
scrubbed the now-dangling reference. That's not something that would show up in
...
indefinitely at day ≥280 rather than resolving some other way. That's
documented as a known limitation, not fixed further, since a graceful
fallback (birth without a recorded father? cancel the pregnancy?) is a
real design decision, not a bug fix, and hasn't come up in practice.
...
mp_pregnant_stage_N`. That's genuinely invisible, not just visually
...
correctly ("A Child is Born"). Male-male conception, pregnancy, and birth are now
...
- **Naming widget**: `character_event.gui` genuinely contains a
  ...
  instead of firing it bare/immediately. That meant the child reference
...
- Seriously considered switching to `father =` + `real_father =` (both
  ...
  (`common/scripted_triggers/00_bastard_triggers.txt:76`), and using this
...
track record this session (systematic-debugging: 2+ failures means an
...
actually recorded as `father` gets the prompt. The trade-off is the carrier
...
7. **Brainstorm the flavor-content direction before implementing
   ...
   then concrete approaches, then approval, then implementation, not
   guessing at scope). Per the user (2026-08-15): once the top-priority
   items above are confirmed working, the mod is considered close to
   release-ready, and this is the next phase.
